...
pub trait Lens<T: ?Sized + Diffable, U: ?Sized + Diffable> {
    ...
    fn outer_diff(&self, u: U::Diff) -> T::Diff;
    // this will get called if inner has changed
    fn with_inner_diff<R>(&self, t: &T::Diff, f: impl FnOnce(&U::Diff) -> R) -> R;
}
