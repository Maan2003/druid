...
impl Slider {
    ...
    fn calculate_value(&self, mouse_x: f64, knob_width: f64, slider_width: f64) -> f64 {
        ...
        if let Some(stepping) = self.step {
            let last_step = self.min + ((self.max - self.min) / stepping).floor() * stepping;
            // max is not regular
            if value > last_step {
                // round the value to whatever is closest, last_step or max
                value = if value - last_step < self.max - value {
                    last_step
                } else {
                    self.max
                };
            } else {
                // round value to the closest step
                value = self.min + ((value - self.min) / stepping).round() * stepping;
            }
        };
        value
    }
